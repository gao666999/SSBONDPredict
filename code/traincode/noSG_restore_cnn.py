...
parser.add_argument('--checkpoint_dir', type=str, default='/Users/dongxq/Desktop/SSBOND_supplement/rootpath40/CNNlog/',
...
def test(sess,images,labels,logits,out):
	...
	print('accuracy is :kkkkkkkkkkkkk%.2f'% accuracy_)
...
def predict(sess,images,labels,logits,out):
	...
	print(data.reshape(-1,10,10,1).shape)
	out_ = sess.run(out,feed_dict={images:data.reshape(-1,10,10,1)})
...
def newpredict(sess,images,labels,logits,out):
...
def predict_one_map(sess,images,file,out):
	data = np.load(file).reshape((-1,10,10,1))
...
def main(argv=None): 
	...
	newpredict(sess,images,labels,logits,out)
