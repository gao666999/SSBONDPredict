...
def main(unused_argv):
    ...
    if FLAGS.predict == False:
        ...
        convert_to_shulffle_tfrecord(train_dataset, 'cullpdb_train', labelmark)
        convert_to_shulffle_tfrecord(test_dataset, 'cullpdb_test', labelmark)
    else:
...
if __name__ == '__main__':
    ...
    newpath=''
    for partpath in old_abspath[:-3]:
        newpath=newpath + partpath +'/'
    new_abspath=newpath+'Dataset/detaildata/'
    parser = argparse.ArgumentParser()
    parser.add_argument(
      '--directory',
      type=str,
      default=new_abspath,
      help='Directory to exist positive and negative data files and write the converted result'
    )
    parser.add_argument(
      '--p_sample',
      type=str,
      default='positive_distance_map.npy',
      help='Directory to exist positive data files and write the converted result'
    )
    parser.add_argument(
      '--n_sample',
      type=str,
      default='negative_distance_map.npy',
